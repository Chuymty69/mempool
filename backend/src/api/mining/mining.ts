...
class Mining {
  ...
  public async $generatePoolHashrateHistory(): Promise<void> {
    ...
    try {
      ...
      if (newlyIndexed > 0) {
        logger.notice(`${logger.tags.mining} Weekly mining pools hashrates indexing completed: indexed ${newlyIndexed}`);
      } else {
        logger.debug(`${logger.tags.mining} Weekly mining pools hashrates indexing completed: indexed ${newlyIndexed}`);
      }
  ...
  public async $generateNetworkHashrateHistory(): Promise<void> {
    ...
    try {
      ...
      if (newlyIndexed > 0) {
        logger.notice(`${logger.tags.mining} Daily network hashrate indexing completed: indexed ${newlyIndexed} days`);
      } else {
        logger.debug(`${logger.tags.mining} Daily network hashrate indexing completed: indexed ${newlyIndexed} days`);
      }
      loadingIndicators.setProgress('daily-hashrate-indexing', 100);
    } catch (e) {
      loadingIndicators.setProgress('daily-hashrate-indexing', 100);
      logger.err(`${logger.tags.mining} Daily network hashrate indexing failed. Trying again in 10 seconds. Reason: ${(e instanceof Error ? e.message : e)}`);
      throw e;
    }
  }
  ...
  public async $indexDifficultyAdjustments(): Promise<void> {
    ...
    if (totalIndexed > 0) {
      logger.notice(`${logger.tags.mining} Indexed ${totalIndexed} difficulty adjustments`);
    } else {
      logger.debug(`${logger.tags.mining} Indexed ${totalIndexed} difficulty adjustments`);
    }
  }
