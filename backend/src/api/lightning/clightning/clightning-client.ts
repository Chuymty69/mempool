...
export default class CLightningClient extends EventEmitter implements AbstractLightningApi {
  ...
  constructor(rpcPath = defaultRpcPath) {
    ...
    if (!fExists(rpcPath) || !fStat(rpcPath).isSocket()) {
      ...
      // to be removed in v0.2.0
        ...
        logger.warn(`${logger.tags.ln} ${rpcPath}/lightning-rpc is missing, using the bitcoin mainnet subdirectory at ${rpcPath}/bitcoin instead.`)
        logger.warn(`${logger.tags.ln} specifying the main lightning data directory is deprecated, please specify the network directory explicitly.\n`)
        rpcPath = path.join(rpcPath, 'bitcoin', 'lightning-rpc')
      }
    }

    logger.debug(`${logger.tags.ln} Connecting to ${rpcPath}`);
    ...
    this.clientConnectionPromise = new Promise<void>(resolve => {
      _self.client.on('connect', () => {
        logger.info(`${logger.tags.ln} CLightning client connected`);
        _self.reconnectWait = 1;
        resolve();
      });

      _self.client.on('end', () => {
        logger.err(`${logger.tags.ln} CLightning client connection closed, reconnecting`);
        _self.increaseWaitTime();
        _self.reconnect();
      });

      _self.client.on('error', error => {
        logger.err(`${logger.tags.ln} CLightning client connection error: ${error}`);
        _self.increaseWaitTime();
        _self.reconnect();
      });
    });
    ...
    this.rl.on('line', line => {
      ...
      // logger.debug(`${logger.tags.ln} #${data.id} <-- ${JSON.stringify(data.error || data.result)}`);
  ...
  reconnect(): void {
    ...
    this.reconnectTimeout = setTimeout(() => {
      logger.debug(`${logger.tags.ln} Trying to reconnect...`);
  ...
  call(method, args = []): Promise<any> {
    ...
    // logger.debug(`${logger.tags.ln} #${callInt} --> ${method} ${args}`);
