   ...
   */
   private async $deleteBlocskToReindex(finalPoolDataUpdate: any[]) {
    ...
    for (const updatedPool of finalPoolDataUpdate) {
      ...
      if (pool.length > 0) {
        logger.notice(`${logger.tags.mining} Deleting blocks from ${pool[0].name} mining pool for future re-indexing`);
        await DB.query(`DELETE FROM blocks WHERE pool_id = ${pool[0].id}`);
      }
    }

    // Ignore early days of Bitcoin as there were not mining pool yet
    logger.notice(`${logger.tags.mining} Deleting blocks with unknown mining pool from height 130635 for future re-indexing`);
    const [unknownPool] = await DB.query(`SELECT id from pools where slug = "unknown"`);
    await DB.query(`DELETE FROM blocks WHERE pool_id = ${unknownPool[0].id} AND height > 130635`);

    logger.notice(`${logger.tags.mining} Truncating hashrates for future re-indexing`);
